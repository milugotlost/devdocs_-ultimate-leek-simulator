...
import {
  ...
  Crown, Printer, Eye, Lock, MessageCircle, X, Shield, Trophy, Trash2
} from 'lucide-react';
        ...
        <div className="h-full w-full bg-yellow-950/30 flex flex-col items-center p-8 overflow-y-auto">
          <div className="max-w-4xl w-full border-4 border-yellow-600 bg-black/90 p-8 shadow-[0_0_50px_rgba(234,179,8,0.3)] my-auto">
            ...
            <div className="text-center pt-8 border-t border-yellow-800">
              ...
              <button
                onClick={() => {
                  if (window.confirm('⚠️ 確定要重製宇宙嗎？\n\n這會清除所有遊戲紀錄：\n• 死亡收藏\n• 成就紀錄\n\n此操作無法復原！')) {
                    localStorage.removeItem('leek_deaths');
                    localStorage.removeItem('leek_achievements');
                    window.location.reload();
                  }
                }}
                className="mt-6 px-6 py-2 border border-red-600 text-red-500 font-bold hover:bg-red-900/30 transition-colors flex items-center gap-2 mx-auto"
              >
                <Trash2 size={16} /> 重製宇宙 (Reset Universe)
              </button>
            </div>
