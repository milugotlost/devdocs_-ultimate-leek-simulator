...
import { 
  ...
  Crown, Printer, Eye, Lock, MessageCircle, X, Shield
} from 'lucide-react';

type GameState = 'START' | 'SELECT_ASSET' | 'SIGN_CONTRACT' | 'PLAYING' | 'GAMEOVER' | 'VIP_MENU' | 'VIP_SELECT';
type Lang = 'en' | 'zh';
...
function App() {
  ...
  const handleTitleClick = () => {
      ...
      if (titleClicks === 4) { // 5th click
          ...
          showNotification(
              "Developer (SysAdmin)", 
              "警告 (Warning)", 
              "別再點了！伺服器過熱！\nStop clicking! Server overheating!"
          );
          setTitleClicks(0);
      }
  };
  ...
  const activateInfiniteMoney = () => {
      ...
      showNotification("Federal Reserve", "Stimulus Package", "Here is $1,000,000,000. Don't ask why.");
  };

  const activateImmunity = () => {
      audio.playClick();
      setHasImmunity(true);
      showNotification("SEC Regulator", "Private Message", "We received your 'donation'. You are now untouchable.");
  };
    ...
    if (turn >= 10) {
      ...
      if (hasImmunity) {
          ...
          showNotification(
              ...
              "We have frozen the market to prevent your losses. Enjoy your bailout."
          );
  ...
  return (
    <div className={`relative h-[100dvh] w-screen ${isCritical ? 'bg-red-900/20' : ''} ${isMatrixMode ? 'hue-rotate-[90deg] contrast-125' : ''} transition-all duration-1000 overflow-hidden flex flex-col`}>
      ...
      <div 
        className={`fixed top-4 left-1/2 transform -translate-x-1/2 w-[90%] max-w-sm z-50 transition-all duration-500 ${notification?.visible ? 'translate-y-0 opacity-100' : '-translate-y-32 opacity-0'}`}
      >
          {notification && (
              <div className="bg-gray-100 border border-gray-300 rounded-2xl shadow-2xl p-3 flex items-start gap-3 text-black font-sans">
          ...
          <div className="h-full w-full bg-yellow-950/30 flex flex-col items-center p-8 overflow-y-auto">
             <div className="max-w-4xl w-full border-4 border-yellow-600 bg-black/90 p-8 shadow-[0_0_50px_rgba(234,179,8,0.3)] my-auto">
                ...
                <div className="text-center pt-8 border-t border-yellow-800">
                    <button 
                        onClick={hardResetUniverse}
                        className="text-xs text-red-500 hover:text-red-400 font-mono underline hover:bg-red-900/20 p-2 rounded"
                    >
                        [ {UI_TEXT.vipReset[lang]} ]
                    </button>
      ...
      {gameState === 'SIGN_CONTRACT' && pendingAsset && (
        <div className="flex items-center justify-center h-full w-full p-4">
           <div className={`max-w-2xl w-full border-2 shadow-2xl flex flex-col max-h-[90vh] ${isVipGameplay ? 'bg-black border-yellow-500' : 'bg-terminal border-term-grid'}`}>
              ...
              <div className="p-4 border-t border-term-grid bg-black flex justify-end flex-shrink-0">
                 <button 
                    onClick={confirmContract}
                    className={`flex items-center gap-2 px-6 py-3 font-bold text-lg hover:bg-white transition-colors ${isVipGameplay ? 'bg-yellow-500 text-black' : 'bg-term-green text-terminal'}`}
                 >
            ...
            <header className="flex justify-between items-end border-b-2 border-term-grid pb-2 flex-shrink-0">
              ...
              <div className="text-right">
                <div className="flex flex-col items-end">
                    ...
                    <div className={`text-2xl md:text-4xl font-bold font-vt323 ${balance < 0 ? 'text-term-red' : (isVipGameplay ? 'text-yellow-500' : 'text-term-text')}`}>
                    {formatMoney(balance)}
                    </div>
                </div>
              </div>
